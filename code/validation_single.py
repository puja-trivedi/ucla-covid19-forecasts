...
parser.add_argument('--dataset', default = "NYtimes",
                    help='nytimes')
parser.add_argument('--dr_state', default = "0.7",
                    help='death rate "a" for state')
...
if __name__ == '__main__':
    ...
    for region in region_list:
        ...
        if args.level == "state":
            # now all states use the same start date, later we can creat a dict to get state-dependent start date
            start_date = "2021-11-26" # start date for omicron variant
            #start_date = "2021-05-30" # start date for delta variant
            state = str(region)
            df_Population = pd.read_csv('data/us_population.csv')
            Pop=df_Population[df_Population['STATE']==state]["Population"].to_numpy()[0]
            
            # GET TRAIN DATA
            train_data = [data.get(start_date, args.END_DATE, state)]
            print("TRAIN DATA BEFORE MODIFICATION ::")
            print("NUM OF CASES: ", train_data[0][0][0:10])
            print("NUM OF DEATHS: ",train_data[0][1][0:10])

            #******************* updated method: subtract one value (currently using ) *************************#
            # base_values are used to normalize the num of cases and deaths for each variant
            base_data = data.get("2021-11-19" , "2021-11-19" , state) # use values from a week prior to start date 
            print("BASE DATA: ", base_data)
            base_value_cases = base_data[0][0]
            base_value_deaths = base_data[1][0]
            # modify number of cases 
            train_data[0][0][:] -= base_value_cases

             # modify number of deaths
            train_data[0][1][:] -= base_value_deaths

            #******************* old method: subtract a range of values (no longer using ) *********************#
            # # edit number of deaths 
            # df_delta = pd.read_csv('delta_pred_state_california.csv', sep=',')
            # train_data[0][1][0:31] = train_data[0][1][0:31] - df_delta['pre_fata'].tolist()
            #****************************************#

            print("TRAIN DATA AFTER MODIFICATION ::")
            print("NUM OF CASES: ", train_data[0][0][0:10])
            print("NUM OF DEATHS: ",train_data[0][1][0:10])

            reopen_flag = True # no longer necessary, keep it here to maintain minimum code change
            # GET FULL DATA
            full_data = [data.get(start_date, args.VAL_END_DATE, state)]
            print("FULL DATA BEFORE MODIFICATION ::")
            print("NUM OF CASES: ", full_data[0][0][:])
            print("NUM OF DEATHS: ", full_data[0][1][:])
            # modify number of cases 
            full_data[0][0][:] -= base_value_cases

             # modify number of deaths
            full_data[0][1][:] -= base_value_deaths

            print("FULL DATA AFTER MODIFICATION ::")
            print("NUM OF CASES: ", full_data[0][0][:])
            print("NUM OF DEATHS: ", full_data[0][1][:])
            ...
            print("VAL DATA BEFORE MODIFICATION ::")
            print("NUM OF CASES: ", val_data[0][:])
            print("NUM OF DEATHS: ", val_data[1][:])
            # modify number of cases 
            val_data[0][:] -= base_value_cases

             # modify number of deaths
            val_data[1][:] -= base_value_deaths

            print("VAL DATA AFTER MODIFICATION ::")
            print("NUM OF CASES: ", val_data[0])
            print("NUM OF DEATHS: ", val_data[1])

            a, decay = 0.7, 0.3  # currently all states use the same decaying parameters
            #a, decay = 0.01, 0.3 # (not original values) values we were testing before we modified training data 
            #pop_in = 1/400 # no longer necessary, keep it here to maintain minimum code change
            pop_in = 0
        elif args.level == "county":
            county, state = region.split("_")
            region = county + ", " + state
        ...
        print("region: ", region, " start date: ", start_date,
    ...
    file_name = "_" + args.level + "_" + args.dr_state
    ...
    write_file_name_all = write_dir + "Single_val_params_END_DATE_" + args.END_DATE + "_VAL_END_DATE_" + args.VAL_END_DATE + file_name
    write_file_name_best = write_dir + "Single_val_params_best_END_DATE_" + args.END_DATE + "_VAL_END_DATE_" + args.VAL_END_DATE + file_name
